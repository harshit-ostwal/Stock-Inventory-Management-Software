...
{
    public partial class FRM_SUPPLIER_IMPORT_EXPORT : Form
    {
        ...
        public FRM_SUPPLIER_IMPORT_EXPORT()
        {
            InitializeComponent();
        ...
        private void LoadProducts()
        {
            try
            {
                string query = "Select f_supplier_id,f_supplier_name,f_gstin,f_mobile_no from Supplier_db";
        ...
        private void FRM_SUPPLIER_IMPORT_EXPORT_Load(object sender, EventArgs e)
        {
            LoadProducts();
        }
        ...
        private void btnDownloadFormat_Click(object sender, EventArgs e)
        {
            using (SaveFileDialog saveFileDialog = new SaveFileDialog())
            {
                ...
                saveFileDialog.FileName = "Suppliers - SS SOFTWARE.xlsx";
        ...
        private void ExportExcel(SaveFileDialog saveFileDialog, bool includeData)
        {
            ...
            {
                ExcelWorksheet productMaster = package.Workbook.Worksheets.Add("Supplier Master");
                ...
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        using (ExcelPackage package = new ExcelPackage(openFileDialog.FileName))
                        {
                            ...
                            dgwDetails.DataSource = dt;
                            MessageBox.Show("Suppliers Imported Successfully! ✅", "SS SOFTWARE", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                    }
                    catch (Exception Ex)
                    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                string query;
                TextBox supplierID = new TextBox();
                if (MessageBox.Show("Do You Wanna Save Data❔", "SS SOFTWARE", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                {
                    if (dgwDetails.Rows.Count > 0)
                    {
                        ...
                        {
                            query = "Select f_supplier_prefix from Supplier_db";
                            con.GetId(query, supplierID);
                            string str = "Select f_supplier_id from Supplier_db Order By ID Desc";
                            con.AutoNumber(str, supplierID);
                            dgwDetails.Rows[i].Cells[0].Value = supplierID.Text;

                            query = "insert into Supplier_db (f_supplier_id,f_supplier_name,f_gstin,f_mobile_no) Values ('" + supplierID.Text + "','" + dgwDetails.Rows[i].Cells[1].Value + "','" + dgwDetails.Rows[i].Cells[2].Value + "','" + dgwDetails.Rows[i].Cells[3].Value + "')";
        ...
        private void FRM_SUPPLIER_IMPORT_EXPORT_KeyDown(object sender, KeyEventArgs e)
        {
            comp.Close(this, e);
        }
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            try
            {
                (dgwDetails.DataSource as DataTable).DefaultView.RowFilter = string.Format("[Supplier Name] LIKE '%{0}%'", txtSearch.Text);
            }
            catch (Exception ex)
            {
