...
namespace SS_SOFTWARE_S.N_JEWELLERS
{
    public partial class FRM_PURCHASE : Form
    {
        ...
        {
            comp.Clear(new Control[] { txtSupplierName, txtInvoiceDate, txtSupplierId, txtGstin, txtMobileNo, txtProductId, txtProductName, txtPrintingName, txtProductCategoryName, txtGodownName, txtBarcode, txtProductSizeId, cmbProductSizeNo, txtProductCategoryName, txtQuantity });
        ...
        private void SaveItems()
        {
            ...
            {
                query = "insert into Purchase_Items_db (f_invoice_no,f_invoice_date,f_product_id,f_product_name,f_product_category_id,f_product_category_name,f_godown_id,f_godown_name,f_barcode,f_product_size_id,f_product_size_no,f_printing_name,f_quantity) Values ('" + txtInvoiceNo.Text + "', '" + txtInvoiceDate.Text + "', '" + row[0] + "', '" + row[1] + "', '" + row[2] + "', '" + row[3] + "', '" + row[4] + "', '" + row[5] + "', '" + row[6] + "', '" + row[7] + "', '" + row[8] + "', '" + row[9] + "', '" + row[10] + "')";
            ...
            using (OleDbConnection connection = new OleDbConnection(Main))
            {
                ...
                if (isRemoveClicked == false)
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        query = "select f_quantity from Purchase_Items_db where f_invoice_no='" + txtInvoiceNo.Text + "' and f_barcode='" + row[6] + "' and f_product_id='" + row[0] + "'";
                        OleDbCommand cmd1 = new OleDbCommand(query, connection);
                        purchaseLastQty = Convert.ToInt32(cmd1.ExecuteScalar());

                        purchaseNewQty = Convert.ToInt32(row[10]);
                ...
                else
                {
                    foreach (DataRow row in dt2.Rows)
                    {
                        ...
                        purchaseNewQty = Convert.ToInt32(row[0]);
        ...
        private void barcodePrinting()
        {
            if (MessageBox.Show("Do You Want To Print Qr Code", "SS SOFTWARE", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    int quantity = Convert.ToInt32(dgwBarcode.Cells[10].Value);
                    for (int i = 0; i < quantity; i++)
                    {
                        dt3.Rows.Add(dgwBarcode.Cells[8].Value, dgwBarcode.Cells[6].Value, dgwBarcode.Cells[9].Value);
                    }
                }
                ds2.Tables.Add(dt3);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (comp.validateControls(textBoxes))
            {
                ...
                {
                    MessageBox.Show("Please Add The Purchasing Products❔👎", "SS SOFTWARE", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    ClearProduct();
                }
            }
            else
        ...
        {
            query = "Select f_product_id,f_product_name,f_product_category_id,f_product_category_name,f_godown_id,f_godown_name,f_barcode,f_product_size_id,f_product_size_no,f_printing_name,f_quantity FROM Purchase_Items_db WHERE f_invoice_no = '" + txtInvoiceNo.Text + "'";
